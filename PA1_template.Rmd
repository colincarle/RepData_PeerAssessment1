...
* The mean of the total number of steps taken per day is `r mean(dailySteps$steps)`, denoted by the solid line above.
* The median of the total number of steps taken per day is `r median(dailySteps$steps)`, denoted by the dashed line above.
...
* Below is a time series plot of the number of steps taken versus the 5-minute intervals, averaged across all days.
    ...
    print(g1)
    ```
* The 5-minute interval containing the maximum number of steps is `r activityPattern[which.max(activityPattern$steps), 1]`, as denoted by the vertical line in the plot above.

## Imputing missing values

* There are `r nrow(molten)` observations in the dataset, `r sum(is.na(molten$value))` of which are missing values.
...
* The imputed data is recast and a new histogram is generated.
    ...
    print(hI)
    ```
* The mean of the imputed data is `r mean(dailyStepsI$steps)`, represented by the solid red line above.
* The median of the imputed data is `r median(dailyStepsI$steps)`, represented by the dashed black line above.
